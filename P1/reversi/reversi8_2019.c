...
extern int patron_volteo(uint8_t tablero[][DIM], int *longitud, uint8_t FA, uint8_t CA, uint8_t SF, uint8_t SC, uint8_t color)
{
...
int actualizar_tablero(uint8_t tablero[][DIM], uint8_t f, uint8_t c, uint8_t color)
{
    ...
    int i, flip, patron;
    //volatile unsigned int lectura = 0;
   // timer2_inicializar();
   // timer2_empezar();
